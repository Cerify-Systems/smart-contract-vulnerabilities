    ...
    OwnableUpgradeable
{
    ...
    function updateAvailabilityForNFT(address nft, bool isAvailable) external onlyOwner {
        require(nft != address(0), "address 0 is not allowed");
    ...
    function updateFee(
        ...
        require(projectCreationFee <= 1 ether, "limit projectCreationFee");
        require(lpTokenNormalLockFee <= 1 ether, "limit lpTokenNormalLockFee");
        require(lpTokenVestingLockFee <= 1 ether, "limit lpTokenVestingLockFee");
        require(normalTokenNormalLockFee <= 1 ether, "limit normalTokenNormalLockFee");
        require(normalTokenVestingLockFee <= 1 ether, "limit normalTokenVestingLockFee");
        fee = Fee({
    ...
    function lockLpV3(
        ...
        address projectToken,
        address referrer
    ) external payable override nonReentrant validNFT(nftManager) returns (uint256 id) {
    ...
    function _multipleLock(
        ...
        {
            require(owners.length <= 200, "200 addresses allowed at most.");
    ...
    function unlock(uint256 lockId) external override validLock(lockId) nonReentrant isLockOwner(lockId) {
        Lock storage userLock = _locks[lockId];
    ...
    // function unlockAllAvailable() external nonReentrant {
    ...
    function editLock(
    ...
    ) external override validLock(lockId) nonReentrant isLockOwner(lockId) {
        Lock storage userLock = _locks[lockId];
        ...
        if (userLock.nftManager == address(0)) {
            if (additionalAmount > 0) {
                ...
                userLock.amount += amountIn;
                CumulativeLockInfo storage tokenInfo = cumulativeLockInfo[
                    userLock.token
                ];
                tokenInfo.amount = tokenInfo.amount + amountIn;
    ...
    function increaseLiquidityCurrentRange(
    ...
    ) external validLock(lockId) isLockOwner(lockId) validLockLPv3(lockId) nonReentrant returns (uint128 liquidity, uint256 amount0, uint256 amount1) {
        Lock storage userLock = _locks[lockId];
        require(userLock.unlockedAmount == 0, "Lock was unlocked");
        (
            ,
        ...
        uint256 amountIn0 = _safeTransferFrom(
            token0,
            msg.sender,
            address(this),
            amount0ToAdd
        );
        uint256 amountIn1 = _safeTransferFrom(
            token1,
            msg.sender,
            address(this),
            amount1ToAdd
        ...
        IERC20(token0).forceApprove(address(userLock.nftManager), amountIn0);
        IERC20(token1).forceApprove(address(userLock.nftManager), amountIn1);
    ...
    function decreaseLiquidityCurrentRange(
        uint256 lockId,
        uint128 liquidity
    ) external validLock(lockId) isLockOwner(lockId) validLockLPv3(lockId) nonReentrant returns (uint256 amount0, uint256 amount1) {
        Lock storage userLock = _locks[lockId];
    ...
    function editLockDescription(
        uint256 lockId,
        string memory description
    ) external validLock(lockId) nonReentrant isLockOwner(lockId) {
        Lock storage userLock = _locks[lockId];
